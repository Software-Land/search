...
Worker searches use the same ordinary `SearchEngine.search()` path as in-process search, including 0.6.5 packed ranking evidence when eligible and fail-closed Stage-2A / Stage 3A pruning when those proofs apply. They forward result/related rows plus a high-level timing/count meta subset (`totalMs`, `retrieveMs`, `featureMs`, `selectionMs`, `rankMs`, `candidateCount`, `matchCount`, `relatedCount`, `relationshipStrategy`). Stage-2 posting/block/pruning counters stay on `searchDetailed()` / `lastSearchMeta` and an internal Worker diagnostics switch; they are not part of the public Worker protocol. `explain: true` stays on the diagnostic FeatureVector path and still preserves each returned row's explanation, exact successor, and constraint metadata.
