...
- unoccupied unique configured-form prefixes attach `configuredPrefixRecall` instead of occupancy. Ordinary lexical retrieval stays active. Weak recall admits the concept **key only** (provenance `configured-prefix-recall`) and scores `directClass` none prefix-only candidates with `configuredPrefixRecallScore` × the existing 0.25 body-lexical coefficient. Key-only hits that already have ordinary lexical class do not receive that addend. Ambiguous prefixes fail closed. There is no hard 1/N coverage cutoff. Packed ranking evidence fails closed for this query class (`configured-prefix-recall`) and uses the FeatureVector path.
