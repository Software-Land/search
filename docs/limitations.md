...
- **Compiled-index memory remains object-heavy.** `search-v2-lexical-index` v1 stores compact positional rows on disk, but the runtime currently reconstructs per-document token arrays, sets, and position maps for the unchanged feature extractor while retaining the compiled posting view. This duplicates some query-independent lexical information. It is the Stage-1 correctness baseline, not the final memory layout. Compact/lazy feature views and mapped artifacts remain future work.
