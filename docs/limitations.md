...
- **High-DF posting work is still Θ(matches).** The exact indexed retriever enumerates every legitimate match and performs no WAND, MaxScore, posting-block skipping, or posting early termination. Stage 2A can reject full feature evaluation for proven plain single-token body-only candidates, but multi-term/prefix/configured/version and uncertain cases remain exhaustive. `candidateLimit` is no longer an exactness bound.
