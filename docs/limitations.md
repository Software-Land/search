...
- **High-match multi-token / phrase queries still fully extract features for every legitimate match.** Exact `directClass` and builtin constraint-signature membership can themselves depend on body evidence (`bodyLexicalMatch`, body vs title `phraseAdjacency`, `bodyPhraseCount`). There is no production lazy FeatureVector evaluator and no production score-bound rejection. A title-first remaining-score bound is investigation evidence only; see [lazy-features.md](lazy-features.md). One mixed N=25k Node observation of `"virtual private network"` was about 10,041 matches and about 276 ms p50. That is benchmark evidence from that generator, seed, and machine, not an SLA or a stable performance claim.
...
Conservative posting-entry block pruning beyond Stage-2B identical-list skip, production lazy FeatureVector evaluation, incremental updates, native ports. Do not treat those as promised. The practical browser target of about 10k–25k documents is an engineering/performance target, not a quality cliff; see [scaling.md](scaling.md) and [compact-runtime.md](compact-runtime.md).
