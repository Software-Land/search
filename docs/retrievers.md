...
Identical inputs serialize byte-identically with `JSON.stringify`. A supplied artifact is checked for format/version, integrity, core analyzer identity, lemma identity, schema fields, document count, and a fingerprint of ids plus searchable title/body text and lexical-frequency data. Optional `summary` is not part of that v1 fingerprint; it is hydrated from the caller documents at load. Re-index after a consumed artifact additionally checks a hydration fingerprint so a summary-only change cannot reuse stale summary state. An invalid supplied artifact throws; it is never ignored in favor of an approximate path. A custom lemma plugin without a deterministic `indexIdentity` remains valid for artifact-omitted runtime construction but is rejected when a supplied artifact cannot prove analyzer compatibility.
