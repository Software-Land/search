...
- Indexed Settings-like ~100k stayed tens of milliseconds; full-scan ranking of a high-DF query previously exhausted heap by retaining Θ(C²) pair diagnostics. 0.3.1 removes that allocation and stores remaining directed edges as packed uint32 chunks. Comparison time remains Θ(C²) and is unsuitable for typeahead at 10k+; use `indexed` / `adaptive`.
