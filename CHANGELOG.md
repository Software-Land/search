...
- `mergeRelationships(base, extra)` merges two `search-v2-relationships` artifacts without mutating caller objects. Same source/target/type keeps the first edge. Use it when combining generated semantic neighbors with `authored.relationships`. When there is no generated artifact, pass `authored.relationships` to `SearchEngine.create`.
