...
- Unique exact left prefixes occupy only when coverage is at least 1/2 (`national institute` 2/4 occupies NIST; `basically available` 2/6 stays graded BASE recall). Character-prefix final tokens keep the older 2/3 floor. Occupancy coverage counts content tokens only. A stop cannot supply the content that crosses the occupancy threshold (`identity and` stays IAM recall). Skipped trailing structural tokens, including proper prefixes of skippable stops, are occupancy-transparent, so `national institute o` / `national institute of` keep NIST occupancy instead of minting `o` as ordinary lexical content or downgrading to recall. Trailing stops under unoccupied recall are not independent lexical terms, so `national in` cannot crowd out TLS.
- A one-token proper prefix of a unique first form token is graded recall, never occupancy. `nationa` is weaker NIST recall than exact `national`. Exact first tokens that belong to more than one concept (`hypertext`, `real`, `software`) stay fail-closed. Proper prefixes that match several concepts (`appl`, `hypertex`) still do not occupy and still have no unique `configuredPrefixRecall`; retrieval keeps every matching key as weak key-only evidence. Leading wrapper stops are skipped for prefix recall only. The complete-interpretation collector does not treat an ambiguous unigram prefix as a complete body-phrase interpretation. Configured *key* prefixes remain a separate occupancy path.
...
- Independently retrieved configured-prefix-recall candidates stay `direct` when a document relationship is later attached. Relationship overlay is corroborating provenance. Neighbors admitted only through `relationship` remain `related`. Equivalent/standalone/topical none-class neighbors with a relationship keep the existing `related` classification.
- Occupancy-era first-token stubs such as `appl` still do not occupy API. Product ranking must not fall through to corpus-ordered body prefixes. Ambiguous proper prefixes retrieve every matching configured key as weak recall; complete-interpretation then keeps those keys instead of spraying body PhrasePrefix hits. If those keys do not retrieve any title-key document, the collector does not apply an empty cohort. Title-key prefix-recall still receives configured title identity (`key-in-title`) and title tightness of that key so definition pages outrank incidental key mentions. Hybrid expands authored neighbors from every strong matching key in an ambiguous group, not only the top title. Single-token title prefixes such as `zer` stay alongside that cohort. Ranking weights are unchanged.
...
- Documentation and test-harness-only patch. No runtime, search, ranking, retrieval, result, API, or artifact behavior changes.
...
- Test fixtures used by the Software.Land scenario generator are now explicit ES modules (`.mjs`), restoring the Node 18 CI lane without changing package runtime behavior.
...
- Optional schema role `summary` for authored short text. Existing title/body callers are unchanged. Summary participates in positional PhraseQuery / PhrasePrefixQuery, configured-concept field evidence, phrase ranking features, and the optional complete-interpretation collector. Ordinary unigram/token candidate generation remains title and body. The v1 lexical-index artifact still stores title/body postings only; summary is hydrated from caller documents.
- Positional query clauses: PhraseQuery (exact contiguous originalSurface), PhrasePrefixQuery (exact preceding tokens plus a final-token prefix), and a configured-form token DAG when typed configured alternatives exist. These record field hits; they do not collapse the default public list.
- Optional `resultCollector: "complete-interpretation"`: keep documents that fully match an executed PhraseQuery or PhrasePrefixQuery, plus already-featured independent authored title evidence. Occupancy, configured-content identity, and version queries decline the collector. Enablement is the caller's, not Core default policy. Rarity/DF thresholds are not the mechanism.
- Title or summary exact typed phrase is strong lexical field evidence and outranks incidental one-token title overlap via the existing repeated-phrase-over-weak-direct constraint class. Markdown/body-only exact phrases stay weak lexical and do not beat title-grade structured/equivalent/topical/version support.
...
- Custom Worker hosts: 0.4 `createWorkerRuntime({ dictionary })` with `dictionary({ entries })` should migrate to `createWorkerRuntime({ compileAuthoredRelevance })` using the root-exported `compileAuthoredRelevance({ configuredConcepts, relationshipMap, documents })`. That is the supported public 0.5 authored-relevance compiler. There is no compatibility alias for `dictionary`. `compileConfiguredConceptPlugin` is an internal/lower-level configured-concept compiler, not a root public export, and is not the normal public migration path.
