...
- Configured concepts are authored as `{ key, aliases }` plus optional identity metadata (`type`, `provenance`, `confidence`). `aliases[0]` is the canonical lexical sequence and is compiled internally as the existing expansion sequence kind. Alternate same-intent forms are `aliases[1…]`. The public authoring schema no longer has `exp` / `expansion`, `primary`, `standaloneRecall`, or `topicalRecall`. `migrateConfiguredEntry()` emits `{ key, aliases }` only.
- Explicit application-owned relevance is authored in directional `relationshipMap` with kinds `equivalent` and `related`, and typed endpoints `{ form } | { concept } | { document }`. Edges do not auto-reverse and must not carry numeric weights. `equivalent` compiles onto the existing one-hop recall machinery. `related` token→concept compiles onto existing standalone-recall semantics. `related` concept→form compiles onto existing topical-recall semantics. `related` document→document compiles onto existing editorial relationship artifact semantics (`type: editorial`, provenance `manual`, strength 1).
...
- `compileAuthoredRelevance({ entries, relationshipMap, documents })` is the public application compiler. It installs dictionary identity, equivalent one-hop recall (result field `synonyms`), related standalone/topical forms, and editorial document edges. Explain provenance may still say `standaloneRecall` / `topicalRecall` because those remain compiled runtime/explain names, not authoring fields. `compileRelationshipMap()` remains a lower-level/partial compiler for tooling and is not the preferred full application initialization path.
- Public `dictionary()` options are `{ entries }` only. Passing `relationshipMap` / `documents` is not a complete authored-relevance path and is not part of the public factory contract.
...
- Authored `equivalent` relationshipMap edges compile to a directional one-hop recall plugin (`authored.synonyms`). Object-map entries are one-hop and directional; they do not auto-reverse. Legacy `{ format, entries: [{ terms }] }` compiled `search-v2-synonyms` artifacts stay bidirectional via `parseSynonyms()`. Lookup may use a unique configured key/span, an exact lexical phrase source, or uncovered tokens. It does not rewrite typed identity, lexical intent, or configured occupancy, and it does not activate topical or standalone recall.
...
- Reviewed standalone recall hints that can broaden retrieval for exact literal queries without changing their configured or lexical intent.
- Reviewed one-hop topical recall declarations on configured dictionary entries. Trusted configured identity can retrieve related topical phrases without rewriting query tokens, lexical intent, or configuredSequenceIntent. Provenance is `topical-recall`. Stage 3A fails closed. Unique exact configured subspans may activate the same topical path when every remaining token is already a `DEFAULT_STOP` word and whole-query `configuredSequenceIntent` is absent. Incomplete/prefix spans, non-stop remainders, and distinct multi-key spans fail closed.
...
- Packed Chromium GitHub Actions gate `software-land-real-corpus-browser` runs the same historical contracts through `SearchClient` and a real Worker loaded from the packed npm tarball (`test/chromium-pack/software-land-scenarios.mjs`). Fixture-only; no Software.Land checkout, Gatsby, or secrets.
- Software.Land fixture `dictionary.json` now includes the later committed `acronymMap.js` graphics/FPS cluster (`fps` → `frames per second`, plus `webgl` / `webgpu` / related keys) so packed-browser `frames per` occupies 200FPS at moderate. Documents, lemmas, relationships, and the dict-only exact-output oracle identity are unchanged. Manifest `corpusSourceCommit` / `searchPackageVersion` remain the original 0.3.1 corpus export; `dictionaryAcronymMapSoftwareLandCommit` records Software.Land `df852eb4136dc5fb5b23cbf0bc22d45170e71423` as the later `acronymMap.js` overlay source.
...
- Directional search equivalences authored on a single-token inflected form remain reachable from that form's canonical lemma-table identity. Lookup is built once at plugin bind from `canonicalLemma` only; heuristic suffix stems are not used. Exact authored keys stay authoritative. Distinct target sets that collapse to the same lemma fail closed. Authored maps are not mutated, reversed, or expanded with a literal `shard` key. Multi-token morphology folding is unchanged.
...
- Related neighbors no longer forcibly outrank a weak-direct candidate whose body matches a directed target from an ordinary same-concept search equivalence. The candidate-local `ordinaryEquivalenceBodyMatch` provenance flag is distinct from repeated body evidence, adds no score, and leaves coverage/direct-class semantics unchanged.
...
- Configured expansion/alias alignment may skip interior typed stop words (`of`, `and`) without rewriting typed surface or stripping stops from the query representation. The last typed content token may use the existing last-token prefix rules. A unique expansion suffix of at least three content tokens may occupy when it includes the final expansion token; one-token fragments and non-stop gaps fail closed. Dictionary `primary` remains unused.
- An explicit unique 1-token expansion-word alias may occupy whole-query configured intent on exact typed identity only. Prefix stubs, interior spans, and colliding aliases fail closed. Dictionary `primary` remains unused.
- Compact significant-symbol query tokens now project their spoken lexical phrase as a retrieval alternative of the same typed concept. The existing operator map (`*` → star, `+` → plus, `#` → sharp) is reused; typed surface stays `a*` while `a star` can match a title tokenized as `a` `star`. Search-equivalence `a* → astar` is unchanged. Bare punctuation and ordinary separators are not spoken-expanded.
...
- A valid morphology lemma may occupy an exact configured key (`apis` → `api`) without rewriting typed surface, using typo alternatives, or looking up keys fuzzily. Unique n≥2 left-prefixes of a configured expansion set `configuredSequenceIntent` so partial phrases such as `application programming` project the canonical expansion as lexical evidence. Ambiguous one-token first-expansion prefixes occupy the unique longest matching expansion (`appl*` → API over AppSec) and fail closed on same-length ties; unique configured-key prefixes such as `app` → AppSec stay on the existing key-prefix path. Wrapped stop remainders reuse the same occupancy. Dictionary `primary` is still unused.
...
- Authored `relationshipMap` sources and directional synonym-map keys are stored on prototype-free records. Ordinary inherited names such as `toString` compile as data. Whitespace-normalized `__proto__` / `prototype` / `constructor` are rejected after the same trim used for the key.
