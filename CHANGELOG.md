...
- Public TypeScript plugin types `DictionaryPlugin` and `SynonymPlugin` are removed. `SearchPlugin` is the public plugin contract. `compileAuthoredRelevance()` returns ordered `SearchPlugin[]` for configured identity and authored relevance. Public `SearchPlugin` no longer includes `expand`. Equivalent recall is authored as `relationshipMap` `kind: "equivalent"` and compiled with `compileAuthoredRelevance()`.
...
- Occupied configured concepts in explain output now use `query.concepts[].kind: "configured-concept"` instead of `"acronym"`. The old value was the historical tag for configured identity and did not imply that the configured concept was literally an acronym. There is no compatibility alias.
...
- `@software-land/search/corpus` compiled `configuredConcepts` are `{ key, aliases }` with `aliases[0]` canonical. Application rows passed to `normalizeExternalConfiguredConcepts` use the same shape. Miner-internal expansion comparison is unchanged. `compileCorpus()` returns `configuredConcepts` and `configuredConceptArtifact` directly; `dictionaryEntriesFromEquivalences` is removed.

### Added

- `@software-land/search/corpus` exports `normalizeExternalConfiguredConcepts` and `ExternalConfiguredConceptError` for application-generated `{ key, aliases }` rows (`aliases[0]` is canonical). The compiler does not call a model and does not accept a multi-sense `expansions: []` public schema. Use `migrateConfiguredEntry()` for a one-shot conversion from `{ key, expansion, aliases, primary }`.
...
- `classifyExpansionRelation` treats British/American suffix spelling (`acknowledgement`/`acknowledgment`, `colour`/`color`, `optimisation`/`optimization`) and conservative short-form abbreviations of an aligned longer token (`tech`/`technical`) as compatible. Distinct meanings (`authentication`/`authorization`, continuous delivery vs continuous deployment) stay unresolved.
