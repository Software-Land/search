...
- Authored `relationshipMap` `equivalent` edges are directional one-hop recall. They do not auto-reverse. `compileAuthoredRelevance()` installs that recall in `authored.plugins`; there is no separate root `synonyms()` application-authoring API. Lookup may use a unique configured key/span, an exact lexical phrase source, or uncovered tokens. It does not rewrite typed identity, lexical intent, or configured occupancy, and it does not activate topical or standalone recall. Legacy `{ format, entries: [{ terms }] }` `search-v2-synonyms` artifacts stay bidirectional via `parseSynonyms()`. Low-level `authored.synonyms` remains the compiled `SynonymPlugin` for inspection.
- `mergeEditorialRelationships(base, editorial)` merges authored editorial document edges onto a base `search-v2-relationships` artifact without mutating caller objects. Use it when combining generated semantic neighbors with `authored.editorialRelationships`. When there is no generated artifact, pass `authored.relationships` to `SearchEngine.create`.
