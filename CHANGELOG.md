...
- Software.Land fixture `dictionary.json` now includes the later committed `acronymMap.js` graphics/FPS cluster (`fps` → `frames per second`, plus `webgl` / `webgpu` / related keys) so packed-browser `frames per` occupies 200FPS at moderate. Documents, lemmas, relationships, and the dict-only exact-output oracle identity are unchanged.
