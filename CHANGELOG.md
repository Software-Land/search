...
- A one-token proper prefix of a unique first form token is graded recall, never occupancy. `nationa` is weaker NIST recall than exact `national`. Ambiguous first-token prefixes such as `hypertex` / `hypertext` fail closed. Configured *key* prefixes remain a separate occupancy path.
...
- Software.Land historical `expectedTop` for occupancy-era first-token stubs such as `appl` / `what is an appl*` no longer requires API identity ranking. That #1 was longest-form occupancy of `application programming interface`, which violated monotonicity. Ambiguous first-token prefixes fail closed; remaining API-family lexical hits such as REST API vs GraphQL still participate. Ranking weights are unchanged.
