...
- One-shot `migrateConfiguredEntry(old)` converts `{ key, exp|expansion, aliases, primary, standaloneRecall, topicalRecall }` into `{ key, aliases }` plus extracted standalone/topical relationship descriptors. `primary` is discarded and is not mapped to any relationship. Runtime `compileAuthoredRelevance()` / `SearchEngine` do not call the helper.
...
- The public configured-identity type is `ConfiguredConcept` (`{ key, aliases?, type?, provenance?, confidence? }`). `EquivalenceEntry` is removed. There is no compatibility alias.
- The versioned configured-identity artifact is `search-v2-configured-concepts` (`ConfiguredConceptArtifact`, `parseConfiguredConcepts`) on `@software-land/search/corpus`. Root no longer exports `EquivalenceArtifact` or `parseEquivalences`. `search-v2-equivalences` is not accepted. 0.4 `search-v2-equivalences` artifacts must be regenerated/migrated for 0.5. The compiler writes `configured-concepts.json`.
- Public TypeScript plugin types `DictionaryPlugin` and `SynonymPlugin` are removed. `SearchPlugin` is the public plugin contract. `compileAuthoredRelevance()` returns ordered `SearchPlugin[]` for configured identity and authored relevance. Public `SearchPlugin` no longer includes `expand` or compiled related-recall maps (`standaloneRecallByToken`, `topicalRecallByKey`). Public `SearchPlugin` no longer exposes `sequences` or `byKey`. Those fields are compiled configured-identity internals. Author configured identity through `configuredConcepts` and `compileAuthoredRelevance()`. Published 0.4 `SearchPlugin` included both fields; runtime compiler plugins may still carry them internally. JavaScript duck typing of those compiler products is not the public authoring contract. Equivalent recall is authored as `relationshipMap` `kind: "equivalent"` and compiled with `compileAuthoredRelevance()`. Related recall is authored as `relationshipMap` `kind: "related"`.
...
- Occupied configured concepts in explain output now use `query.concepts[].kind: "configured-concept"` instead of `"acronym"`. The old value was the historical tag for configured identity and did not imply that the configured concept was literally an acronym. Experimental `meta.stage3AFallbackReason` also uses `"configured-concept"` instead of `"acronym"` when configured occupancy is the failing Stage 3A check. There is no compatibility alias.
- Explain ranking feature `configuredEquivalenceMatch` is now `configuredConceptMatch`. Values remain `false`, `"key-in-title"`, or `"expansion"`. The field is configured-concept title identity, not `relationshipMap` `kind: "equivalent"`. There is no compatibility alias.
...
- The low-level `dictionary()` root factory has been removed from the public API. Configured concept authoring uses `compileAuthoredRelevance({ configuredConcepts, ... })`; the internal dictionary plugin remains an implementation detail (`name: "dictionary"`). 0.4 identity-only `SearchEngine.create({ plugins: [morphology(), dictionary({ entries })] })` or a custom `SearchPlugin` typed with `sequences` / `byKey` becomes `plugins: [morphology(), ...compileAuthoredRelevance({ configuredConcepts: entries }).plugins]`.
...
- `@software-land/search/corpus` compiled `configuredConcepts` are `{ key, aliases }` with `aliases[0]` canonical. Miner-internal expansion comparison is unchanged. `compileCorpus()` returns `configuredConcepts` and `configuredConceptArtifact` directly; `dictionaryEntriesFromEquivalences` is removed.

### Added

- `@software-land/search/corpus` exports `reconcileExternalConfiguredConcepts` and `ExternalConfiguredConceptError` for untrusted/generated configured-concept candidate rows. The helper clusters compatible duplicates, records unresolved ambiguity or conflict, and projects accepted identities to `configuredConcepts: ConfiguredConcept[]` (`aliases[0]` canonical) plus review metadata (`rejected`, `conflicts`, `unresolved`, `reconciliations`). Candidate `expansion` is the generated canonical phrase and is not a `ConfiguredConcept` field. Stale fields `primary`, `standaloneRecall`, and `topicalRecall` are rejected; use `migrateConfiguredEntry()` for 0.4 rows and `relationshipMap` `kind: "related"` for related recall. The compiler does not call a model and does not accept a multi-sense `expansions: []` public schema.
...
- Indexed and compiled retrieval preserve full-scan prefix recall for recall-derived forms. Unambiguous configured-expansion prefixes that already meet the occupancy information bound keep their intended direct-evidence class: a 2/3 exact left prefix of a uniquely resolved expansion is moderate, not merely weak body overlap. The existing prefix information bound is unchanged.
