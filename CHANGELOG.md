...
- The versioned configured-identity artifact is `search-v2-configured-concepts` (`ConfiguredConceptArtifact`, `parseConfiguredConcepts`) on `@software-land/search/corpus`. Root no longer exports `EquivalenceArtifact` or `parseEquivalences`. `search-v2-equivalences` is not accepted. 0.4 configured-equivalence artifacts must be regenerated/migrated for 0.5. The compiler writes `configured-concepts.json`.
...
- `@software-land/search/corpus` compiled `configuredConcepts` are `{ key, aliases }` with `aliases[0]` canonical. Application rows passed to `normalizeExternalEquivalences` use the same shape. Miner-internal expansion comparison is unchanged. `compileCorpus()` returns `configuredConcepts` and `configuredConceptArtifact` directly; `dictionaryEntriesFromEquivalences` is removed.
