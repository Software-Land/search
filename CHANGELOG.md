...
- Public initialization terminology now distinguishes configured concept definitions from the corpus lexicon and compiled document graph: `dictionaryEntries` is now `configuredConcepts`, and document relationship inputs use `documentRelationships` instead of `relationships`. This applies to `SearchEngine.create`, Worker `SearchClient.init`, `compileCorpus()` results, and `configuredConceptsFromEquivalences` (formerly `dictionaryEntriesFromEquivalences`). The versioned `search-v2-relationships` artifact still stores its graph under the inner `relationships` map.
- `compileAuthoredRelevance({ configuredConcepts, relationshipMap, documents })` is the public application compiler. It returns `{ plugins, documentRelationships }` for direct SearchEngine initialization: `plugins` is the ordered authored relevance plugin collection (configured identity, including related standalone/topical recall, then compiled equivalent one-hop recall), and `documentRelationships` is the editorial document→document artifact or `null`. Explain provenance may still say `standaloneRecall` / `topicalRecall` / `synonym-recall` because those remain compiled runtime/explain names, not authoring fields. `compileRelationshipMap()` remains a lower-level/partial compiler for tooling and is not the preferred full application initialization path.
...
- `@software-land/search/corpus` compiled `configuredConcepts` are `{ key, aliases }` with `aliases[0]` canonical. Application rows passed to `normalizeExternalEquivalences` use the same shape. Miner-internal expansion comparison is unchanged.
...
- Authored `relationshipMap` `equivalent` edges are directional one-hop recall. They do not auto-reverse. `compileAuthoredRelevance()` installs that recall in `authored.plugins`; there is no separate root `synonyms()` application-authoring API. Lookup may use a unique configured key/span, an exact lexical phrase source, or uncovered tokens. It does not rewrite typed identity, lexical intent, or configured occupancy, and it does not activate topical or standalone recall. Legacy `{ format, entries: [{ terms }] }` `search-v2-synonyms` artifacts stay bidirectional via `parseSynonyms()`. Current application authoring does not use that artifact; the corpus compiler may still emit it as miner/review output.
