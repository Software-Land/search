 ...
 * configured concept must produce identical ranked results.
...
describe("Software.Land configured-concept result parity", () => {
  ...
  test("cicd family and paas family share results", () => {
    ...
    const paas = paasForms.map((q) => [q, publicView(engine, plugins, q)]);
    for (const [q, view] of paas) {
      expect({ q, key: view.key }).toEqual({ q, key: "paas" });
      expect(view).toEqual(paas[0][1]);
    }
  });
  ...
  test("same-concept families stay equivalent within indexed and adaptive", () => {
    ...
    for (const retriever of ["indexed", "adaptive"]) {
      for (const forms of families) {
        const views = forms.map((q) => publicView(engines[retriever], plugins, q));
        for (const view of views) expect(view.key).toBe(views[0].key);
        for (let i = 1; i < views.length; i++) expect(views[i]).toEqual(views[0]);
      }
    }
  });
});
