#!/usr/bin/env node
/**
 * search-relationships CLI
 *
 *   node tools/search-relationships/build.mjs analyze --input corpus.json --output dir [--decisions file]
 *   node tools/search-relationships/build.mjs compile --input corpus.json --output dir --decisions file [--semantic file]
...
if (sub === "review") {
  const inspection = /** @type {{ pending?: Array<{ reviewBand?: string | null, id?: string }> } | null} */ (readJson(path.join(output, "inspection.json")));
  if (!inspection) {
