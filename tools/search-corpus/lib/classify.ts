import { documentSupportsPairIndexed, indexDocuments, withinDocumentRepeat } from "./acronyms.js";
import { phraseKey, stableSort } from "./text.js";
...
function summarizeEvidence(row: EquivalenceCandidate, indexedDocs: IndexedDocument[]): EquivalenceEvidence {
  ...
  for (const idx of indexedDocs) {
    ...
    if (withinDocumentRepeat(idx, row.key, row.expansion || [])) withinDocumentRepeats += 1;
  }
  return {
...
export function evidenceForPair(key: string, expansion: string[], documents: CorpusDocument[]): EquivalenceEvidence {
...
export function hasStrongIndependentEvidence(ev: EquivalenceEvidence): boolean {
...
function hasStrongShortTokenEvidence(ev: EquivalenceEvidence, item: Partial<EquivalenceCandidate> = {}): boolean {
  ...
  return hasStrongIndependentEvidence(ev);
}
...
export function classifyCandidates(rawRows: EquivalenceCandidate[], documents: CorpusDocument[]): EquivalenceCandidate[] {
  ...
  for (const [key, items] of byKey) {
    ...
    for (const item of items) {
      ...
      } else if ((ev.withinDocumentRepeats || 0) >= 1) {
        status = "review";
        decision = "within-document-repeat";
