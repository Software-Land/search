...
import { analyzeCorpus, compileAnalysis, compileCorpus } from "./lib/pipeline.js";
import { DecisionError } from "./lib/decisions.js";

const HERE = path.dirname(fileURLToPath(import.meta.url));

/** @param {string} flag @param {string | null | undefined} [fallback] */
...
/** @param {string} dir @param {string} name @param {unknown} obj */
...
if (sub === "review") {
  const inspection = /** @type {import("./types.js").InspectionDoc | null} */ (readJson(path.join(output, "inspection.json")));
  if (!inspection) {
  ...
  let items = inspection.reviewQueue || [
    ...(inspection.pending || []),
...
const previousInspection = sub === "analyze" || sub === "build" || sub === "compile" ? readJson(previousPath) : null;

try {
  fs.mkdirSync(output, { recursive: true });

  if (sub === "analyze") {
    const analysis = analyzeCorpus(input, { decisions, previousInspection });
    const written = [
  ...
  const result = compileCorpus(input, { decisions, previousInspection });
  const written = [
    write(output, "equivalences.json", result.equivalences),
    write(output, "synonyms.json", result.synonyms),
